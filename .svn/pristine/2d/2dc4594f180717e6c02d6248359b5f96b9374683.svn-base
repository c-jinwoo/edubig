...
function fnList() {
	$('#frm').attr('target', '_self').attr('action', '<c:out value="${ctx}"/>/academy/tutor/specialLectureInfo.do').submit();
}
</script>
    ...
    <article class="subArea">
		...
		<div class="path">
            <ul>
                ...
                <li>강사공간</li>
                <li class="active">특강신청정보</li>
            </ul>
        </div>
		<div class="subConts" id="sub-content">
			<section class="contTop">
                <h2 class="h2_tit">특강신청정보</h2>       
            </section>
			
			<form name="frm" id="frm" action="/academy/tutor/specialLectureInfo.do" method="post">
					...
					<table class="sub-table">
						...
						<tbody>
							...
							<tr>
								...
								<td>
								<c:out value="${resultList[0].EDU_SDATE}"/> ~ <c:out value="${resultList[0].EDU_EDATE}"/>
								</td>
							</tr>
							<tr>
								<th scope="row">교육 대상</th>
								<td><c:out value="${resultList[0].LEC_TAGET_NAME}"/><c:if test="${resultList[0].MT_LEC_TARGET_CODE == 'TG0007'}">(<c:out value="${resultList[0].TARGET_ETC}"/>)</c:if></td>
