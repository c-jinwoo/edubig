...
<head>
	...
	<script type="text/javascript">
		...
		//리스트
		function fnList(){
			$('#frm').attr('target', '_self').attr('action', '/bbs/pass/main.do').submit();
		}

		// 저장 및 수정
		function fnSave(){
			if($("#content2").val().replaceAll(" ","").length < 20){
			...
			fnConfirm(msg + "하시겠습니까?", function(result){
				if(result){					
					...
					$('#frm').attr('target', '_self').attr('action', '/bbs/pass/exec.do').submit();
				}
			});
		}		
	</script>
								...
								<div class="board_write">
									<table>
										...
										<tr>
											<th scope="row">분류</th>
											<td>
												<select name="courseno" style="height:30px;padding:0 5px;border:1px solid #ccc;font-size:14px;line-height:18px;">
													<option value="">전체</option>
													<c:forEach items="${courseList}" var="result" varStatus="stat">		
														<option value="<c:out value="${result.COURSENO}"/>" <c:if test="${result.COURSENO eq rowData.COURSENO}">selected</c:if>><c:out value="${result.COURSETITLE}"/></option>
													</c:forEach>							
												</select>
											</td>
										</tr>
