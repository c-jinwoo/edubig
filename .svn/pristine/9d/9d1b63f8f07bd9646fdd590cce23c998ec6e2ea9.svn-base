...
//검색
function fnSearch(){
	$('form[name=frm]').attr('target', '_self').attr('action', '<c:out value="${ctx}"/>/academy/course/sprfndDetail.do');
	$('#frm').submit();
...
function fnCourseApply(courseno, cseqno, cancelYn){
	...
	$('form[name=frm]').attr('target', '_self').attr('action', '<c:out value="${ctx}"/>/academy/course/sprfndForm.do');
	$('#frm').submit();
		...
		<div class="sub-wrap">
			...
			<div class="sub-contents">
				<ul class="sub-navi">
					...
					<li class="location"><span>햇살론</span></li>
				</ul>
				...
				<div class="contents" id="sContents">
						...
						<form name="frm" id="frm" action="/academy/course/sprfndForm.do" method="post">
							...
							<div class="serch-box">
								<input type="text" name="schSeqTitle" title="햇살론 검색어입력" value="<c:out value="${REQUEST_DATA.schSeqTitle}"/>"><button class="btn-submit" onclick="fnSearch(); return false;" title="검색">검색</button>
							</div>
			            </form>
