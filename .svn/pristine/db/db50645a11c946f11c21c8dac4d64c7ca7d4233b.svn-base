...
function fnCourseView(mtScCode){
    ...
    $('form[name=frm]').attr('target', '_self').attr('action', '<c:out value="${ctx}"/>/academy/course/onDetail.do').submit();
}
    ...
    <article class="subArea">
        <div class="subCntWrap">
            ...
            <div class="subConts" id="sub-content">
				...
				<form name="frm" id="frm" action="/academy/course/onCategoryList.do" method="get">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12 col-xs-12">
                                            <div class="crdBox">
                                                <div class="boxInner">
                                                    <dl>
                                                        <dt class="dt_tit"><span class="po_C6 bold fnt19"><c:out value="${result.REF3}"/></span></dt>
                                                        <dd>
                                                            <p><c:out value="${fn:substring(result.SUMMARY,0,56)}"/><c:if test="${fn:length(result.SUMMARY) > 54 }">..</c:if></p>
                                                        </dd>
                                                    </dl>
                                                    <div class="btnAreaC text-right">
                                                       <a href="#none" class="btn btn-lg btn-blue" onclick="fnCourseView('<c:out value="${result.MT_SUB_CODE}"/>'); return false;" <c:if test="${not empty result.MT_SUB_CODE2}">title="<c:out value="${result.REF3}"/> 신청하기" class="btn-m blue abs"</c:if> <c:if test="${empty result.MT_SUB_CODE2}">title="<c:out value="${result.REF3}"/> 상세보기" class="btn-m green abs"</c:if>><c:if test="${not empty result.MT_SUB_CODE2}">신청하기</c:if> <c:if test="${empty result.MT_SUB_CODE2}"> 상세보기 </c:if></a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
