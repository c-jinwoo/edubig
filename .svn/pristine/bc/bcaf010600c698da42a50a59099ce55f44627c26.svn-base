			...
			<div class="contents" id="sContents">
				...
				<div class="con-tbl">
							<table class="sub-table label">
								<caption>강사요청 테이블:이 표는 강사요청 테이블의 지역구분,제목,교육장소,방문여청일시,교육시간,교육대상,인원,신청기간,강의료,신청기관 소재지,교육시설정보,기타내용,강사선택,신청자,신청자연락처 등을 설명하는 테이블입니다.</caption>
								<colgroup>
									<col style="">
								...
								<tr>
									...
									<td><input type="hidden" name="eduTime" value="<c:out value="${map.EDU_TIME}"/>"/><c:out value="${map.EDU_TIME}"/>시간</td>
								...
								<tr>
									...
									<td>
										<input type="text" id="lecPay" name="lecPay" class="inputtxt small" <c:if test="${map.PAY_YN eq 'Y'}">disabled="disabled"</c:if> title="강의료입력" <c:if test="${map.LEC_PAY ne '0'}">value="${map.LEC_PAY}"</c:if> > 원
											...
											<span class="checks">
												<input type="checkbox" id="payYn" name="payYn"  title="" value="Y" title="예산없음"
													<c:if test="${map.PAY_YN eq 'Y'}">
														checked="checked"
													</c:if>
												>
								...
								<tr>
									...
									<td><input type="text" id="eduTarget" name="eduTarget" class="inputtxt small" title="교육대상입력" value="${map.EDU_TARGET}"></td>
								</tr>
								<tr>
									<th scope="row" class="va-t">인원<span class="mark">*<em class="hidden">필수항목</em></span></th>
									<td><input type="text" id="personCnt" name="personCnt" class="inputtxt small" title="인원" value="${map.PERSON_CNT}"> 명</td>
								</tr>
								<tr>
