...
function fnCourseApply(courseno, cseqno, cancelYn){
	...
	$('form[name=frm]').attr('target', '_self').attr('action', '<c:out value="${ctx}"/>/academy/course/enfsnOnForm.do');
	$('#frm').submit();
		...
		<div class="sub-wrap">
			...
			<div class="sub-contents">
				<ul class="sub-navi">
					...
					<li class="location"><span>온라인과정</span></li>
				</ul>
				...
				<div class="contents" id="sContents">
						...
						<form name="frm" id="frm" action="/academy/course/enfsnOnForm.do" method="post">
							...
							<div class="serch-box">
								<input type="text" name="schSeqTitle" title="온라인과정 검색어입력" value="<c:out value="${REQUEST_DATA.schSeqTitle}"/>"><button class="btn-submit" onclick="fnSearch(); return false;" title="검색">검색</button>
							</div>
			            </form>
