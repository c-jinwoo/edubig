...
<script type="text/javascript">
	...
	function move_page(cPage){
		$('#cPage').val(cPage);
		$('form[name=frm]').attr('action', '<c:out value="${ctx}"/>/myclass/learningStatusList.do').submit();
	} 
	...
	function fnClassMove(courseno, searchCseqno){
		...
		$('form[name=frm]').attr('action', '/myclass/learningClassList.do').submit();
	}
	...
	function fnPrint( cuserNo ){		
		...
		var w1 =1070;
		var h1 = 900;
		var url = "/myclass/cerOnlinePrint.do";
		...
		if(chkAt > 0){
			fnConfirm("신청취소 하시겠습니까? " ,function(result){
				if(result){
					...
					$('form[name=frm]').attr('action', '/myclass/del_course_exec.do').submit();
				}
			});
		} else {
			fnModalMsg("선택해주세요.");
